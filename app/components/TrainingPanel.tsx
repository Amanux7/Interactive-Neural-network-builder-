...
export function TrainingPanel({
  ...
  return (
    ...
    >
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="flex items-center gap-2.5 px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-blue-800 to-blue-600 flex-shrink-0">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-blue-100">Mock gradient descent · updates weights</div>
        </div>
        <button
          onClick={onClose}
        ...
        <div className="px-4 pt-4">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            {parsed.data.length > 0 && !parsed.error && (
              <span className="ml-auto text-[10px] text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200 font-medium">
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            {Object.entries(PRESET_DATASETS).map(([key, ds]) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold border transition-all ${selectedPreset === key
          ...
          <textarea
            ...
            rows={3}
            spellCheck={false}
            className="w-full text-[11px] font-mono px-3 py-2 border rounded-xl resize-none focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all bg-slate-50"
            style={{
              borderColor: parsed.error ? '#FCA5A5' : parsed.data.length ? '#93C5FD' : '#E2E8F0',
            }}
              ...
              <div className="grid grid-cols-4 gap-1">
                {TRAIN_SPEEDS.map(s => (
                  <button key={s.value}
                    ...
                    title={s.desc}
                    className={`py-1.5 rounded-lg text-[10px] font-semibold border transition-all ${speedValue === s.value
          ...
          <div className="flex gap-2">
            {isRunning ? (
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-blue-800 to-blue-600 hover:from-blue-700 hover:to-blue-500 text-white rounded-xl text-[12px] font-semibold shadow-sm shadow-blue-300/50 disabled:opacity-40 disabled:cursor-not-allowed transition-all nf-bevel"
              >
          ...
          <div className="px-4 pt-3">
            <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
              ...
              <div className="h-2 bg-slate-200 rounded-full overflow-hidden mb-3">
                <div
                  className={`h-full rounded-full transition-all duration-300 ${isComplete ? 'bg-blue-800' : 'bg-blue-500'}`}
