            ...
            <div className="flex items-center gap-1.5">
              {canApply ? (
                <button
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${styles.barColor}, ${styles.barColor}cc)`,
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 px-4">
      ...
      <div className="relative mb-5">
        ...
        />
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-800 to-indigo-700 flex items-center justify-center shadow-lg relative nf-bevel">
      ...
      <p className="text-[11px] text-slate-400 mb-5 h-4 text-center">
        {SCAN_PHASES[Math.min(phase, SCAN_PHASES.length - 1)]}
      </p>

      {/* Progress bar */}
      <div className="w-48 h-1.5 bg-slate-100 rounded-full overflow-hidden mb-5">
        <div
      ...
      <div className="space-y-1.5 w-48">
        {SCAN_PHASES.slice(0, -1).map((ph, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className={`text-[10px] transition-colors ${i < phase ? 'text-emerald-600' : i === phase ? 'text-indigo-600' : 'text-slate-300'
              }`}>
              {ph}
            </span>
          </div>
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-bold text-white leading-none">NeuralForge AI</div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {applyableCount > 0 && (
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-semibold text-white transition-all hover:opacity-90 nf-bevel"
                  ...
                  {(['all', 'critical', 'warning', 'info', 'tip'] as const).map(sev => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold transition-all ${filter === sev
                            ? 'text-white'
                            : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                          }`}
                        style={filter === sev ? {
                          background: sev === 'all' ? '#1E40AF'
                            : sev === 'critical' ? '#EF4444'
                              : sev === 'warning' ? '#F59E0B'
