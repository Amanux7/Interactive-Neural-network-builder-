...
import {
  Brain, BookOpen, Users, Layers,
  Trash2, HelpCircle, Info, Sparkles,
  Undo2, Redo2, Zap, Wand2, Radio, MessageCircle,
  Moon, Sun, Menu, Download, BarChart2, GraduationCap,
} from 'lucide-react';
...
const STORAGE_KEY = 'neuralforge-v2';
...
export default function NeuralNetworkBuilder() {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' && e.target === document.body) { e.preventDefault(); if (simulation.isRunning && !simulation.isPaused) handlePause(); else handleStart(); }
  ...
  }, [selectedNodeId, simulation, undoStack, redoStack]);
  ...
  return (
    <div
      ...
      style={{ background: isDark ? '#111827' : '#F3F4F6', fontFamily: "'Inter', system-ui, sans-serif" }}
    >

      {/* ── Header ── */}
      <header
        role="banner"
        className="h-14 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700/70
                   flex items-center px-4 gap-2.5 flex-shrink-0 z-50
      ...
      >
        ...
        <button
          ...
          className="lg:hidden w-8 h-8 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center
                     text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex-shrink-0"
        >
          <Menu size={15} />
        </button>
        ...
        <div className="flex items-center gap-2 mr-1 flex-shrink-0">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-800 to-blue-600 flex items-center justify-center shadow-sm shadow-blue-500/30 dark:shadow-blue-900/30 nf-bevel">
            <Brain size={15} className="text-white" aria-hidden="true" />
          </div>
          <div className="hidden sm:block">
            <div className="text-[13.5px] font-bold text-slate-800 dark:text-slate-100 leading-none tracking-tight">NeuralForge</div>
            <div className="text-[9.5px] text-slate-400 dark:text-slate-500 leading-none mt-0.5">Visual Network Builder</div>
          </div>
        </div>

        <div className="h-7 w-px bg-slate-200 dark:bg-slate-700 hidden sm:block flex-shrink-0" aria-hidden="true" />
        ...
        <div className="flex items-center gap-1 flex-shrink-0" role="group" aria-label="History">
          <button onClick={handleUndo} disabled={!undoStack.length}
            aria-label={`Undo${undoStack.length ? ` (${undoStack.length})` : ''}`} title="Undo (Ctrl+Z)"
            className="w-8 h-8 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-slate-700 dark:hover:text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-[var(--shadow-xs)] nf-bevel">
            <Undo2 size={13} />
          </button>
          ...
          {undoStack.length > 0 && <span className="text-[9.5px] text-slate-400 dark:text-slate-500 ml-0.5 tabular-nums">{undoStack.length}</span>}
        </div>

        <div className="h-7 w-px bg-slate-200 dark:bg-slate-700 hidden md:block flex-shrink-0" aria-hidden="true" />

        {/* Templates */}
        <div data-tutorial-id="tutorial-templates" className="hidden md:flex items-center gap-1.5 flex-shrink-0" role="group" aria-label="Example templates">
          <span className="text-[10px] text-slate-400 dark:text-slate-500 mr-0.5 font-medium uppercase tracking-wide">Templates</span>
          {([
            { id: 'mnist', label: 'MNIST', cls: 'bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800/60 hover:bg-blue-100 dark:hover:bg-blue-900/40' },
            { id: 'binary', label: 'Binary', cls: 'bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/40' },
            { id: 'conv', label: 'ConvNet', cls: 'bg-cyan-50 dark:bg-cyan-950/40 text-cyan-600 dark:text-cyan-400 border-cyan-200 dark:border-cyan-800/60 hover:bg-cyan-100 dark:hover:bg-cyan-900/40' },
          ] as const).map(t => (
            <button key={t.id} onClick={() => handleLoadExample(t.id)}
              aria-label={`Load ${t.label} template`}
              className={`px-2.5 py-1.5 text-[11px] font-semibold rounded-lg border transition-colors ${t.cls}`}>{t.label}</button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {/* Summary Dashboard */}
          <button
            data-tutorial-id="tutorial-dashboard"
            onClick={() => setShowDashboard(v => !v)}
            aria-label="Summary Dashboard" aria-pressed={showDashboard} title="Summary Dashboard"
            className={`w-8 h-8 rounded-xl border flex items-center justify-center transition-all shadow-[var(--shadow-xs)] nf-bevel ${showDashboard
              ? 'text-white border-teal-500'
              : 'text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-800/50 bg-teal-50 dark:bg-teal-950/30 hover:bg-teal-100 dark:hover:bg-teal-900/30'
              }`}
            style={showDashboard ? { background: 'linear-gradient(135deg,#0F766E,#0D9488)', boxShadow: '0 2px 12px rgba(15,118,110,0.4)' } : {}}
          >
          ...
          {/* Export Model */}
          <button
            data-tutorial-id="tutorial-export"
            onClick={() => setShowExport(v => !v)}
            aria-label="Export Model" aria-pressed={showExport} title="Export Model (JSON)"
            className={`w-8 h-8 rounded-xl border flex items-center justify-center transition-all shadow-[var(--shadow-xs)] nf-bevel ${showExport
              ? 'text-white border-blue-600'
              : 'text-blue-800 dark:text-blue-400 border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 dark:hover:bg-blue-900/30'
              }`}
            style={showExport ? { background: 'linear-gradient(135deg,#1E40AF,#3B82F6)', boxShadow: '0 2px 12px rgba(30,64,175,0.4)' } : {}}
          >
          ...
          <button
            ...
            aria-label="AI Optimize" aria-pressed={showAIOptimize} title="AI Optimize"
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[11.5px] font-semibold transition-all nf-bevel ${showAIOptimize
              ? 'text-white border-violet-500'
              : 'text-violet-700 dark:text-violet-400 border-violet-200 dark:border-violet-800/50 bg-violet-50 dark:bg-violet-950/30 hover:bg-violet-100 dark:hover:bg-violet-900/30'
              }`}
            style={showAIOptimize ? { background: 'linear-gradient(135deg,#4F46E5,#7C3AED)', boxShadow: '0 2px 12px rgba(99,102,241,0.4)' } : {}}
          >
            <Wand2 size={12} className={showAIOptimize ? 'animate-pulse' : ''} aria-hidden="true" />
          ...
          {collabEnabled ? (
            <div role="group" aria-label="Live session"
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-900 dark:bg-slate-800 border border-slate-700">
              <div className="flex -space-x-1.5">
                <div title="You" className="w-6 h-6 rounded-full border-2 border-slate-900 dark:border-slate-800 bg-emerald-500 flex items-center justify-center text-[8px] font-bold text-white flex-shrink-0">Me</div>
                {collabUsers.map(u => (
                  <div key={u.id} title={u.name} className="w-6 h-6 rounded-full border-2 border-slate-900 dark:border-slate-800 flex items-center justify-center text-[8px] font-bold text-white flex-shrink-0" style={{ background: u.color }}>{u.initials}</div>
                ))}
              </div>
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" aria-hidden="true" />
                <span className="text-[9.5px] font-bold text-emerald-400">LIVE</span>
              </div>
              <button onClick={() => { setChatOpen(v => !v); setUnreadCount(0); }}
                aria-label={`Chat${unreadCount > 0 ? ` (${unreadCount} unread)` : ''}`} aria-pressed={chatOpen}
                className="relative w-6 h-6 rounded-lg bg-slate-700 hover:bg-slate-600 flex items-center justify-center transition-colors">
                <MessageCircle size={11} className="text-white" aria-hidden="true" />
                {unreadCount > 0 && <span aria-hidden="true" className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-500 border border-slate-900 flex items-center justify-center text-[7px] font-bold text-white">{unreadCount > 9 ? '9+' : unreadCount}</span>}
              </button>
              <button onClick={handleToggleCollab} aria-label="Leave session"
                className="text-[9px] font-semibold text-slate-400 hover:text-red-400 transition-colors px-0.5">Leave</button>
            </div>
          ) : (
            <button onClick={handleToggleCollab} aria-label="Start multiplayer session" title="Multiplayer"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[11.5px] font-semibold text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 transition-all nf-bevel">
          ...
          {/* Dark mode toggle */}
          <button onClick={toggleDark} aria-label={isDark ? 'Switch to light mode' : 'Switch to dark mode'} title={isDark ? 'Light mode' : 'Dark mode'}
            className="w-8 h-8 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-amber-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-[var(--shadow-xs)] nf-bevel">
            {isDark ? <Sun size={14} /> : <Moon size={14} />}
          </button>

          <button onClick={() => setShowModelInfo(v => !v)} aria-label="Model summary" aria-pressed={showModelInfo} title="Model summary"
            className="w-8 h-8 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-[var(--shadow-xs)] nf-bevel">
            <Info size={14} />
          </button>
          <button onClick={handleClearCanvas} aria-label="Clear canvas" title="Clear canvas"
            className="w-8 h-8 rounded-xl border border-red-200 dark:border-red-900/50 bg-white dark:bg-slate-800 flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-950/30 transition-all shadow-[var(--shadow-xs)] nf-bevel">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative">
        ...
        {/* Canvas container */}
        <div data-tutorial-id="tutorial-canvas" className="flex-1 h-full overflow-hidden relative bg-[#F3F4F6] dark:bg-[#111827]" ref={canvasContainerRef}>
          <NetworkCanvas
            nodes={nodes}
          ...
          <ExportPanel
            ...
            nodes={nodes} connections={connections} simulation={simulation}
            isDark={isDark}
          />

          {/* Summary Dashboard */}
          <SummaryDashboard
            isOpen={showDashboard} onClose={() => setShowDashboard(false)}
            nodes={nodes} connections={connections} simulation={simulation}
            isDark={isDark}
          />
          ...
          {/* Tutorial Overlay */}
          <TutorialOverlay
            isOpen={showTutorial} onClose={() => setShowTutorial(false)}
            isDark={isDark}
          />
      ...
      {/* ── Simulation Bar ── */}
      <div data-tutorial-id="tutorial-simbar">
        <SimulationBar
      ...
      {/* Welcome modal */}
      {showWelcome && (
        <div role="dialog" aria-label="Welcome to NeuralForge" aria-modal="true"
          className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-[var(--shadow-xl)] max-w-lg w-full p-6 nf-animate-in border border-slate-200 dark:border-slate-700/60">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-blue-800 to-blue-600 flex items-center justify-center shadow-md shadow-blue-500/30 nf-bevel">
                <Brain size={21} className="text-white" aria-hidden="true" />
              </div>
              <div>
                <h1 className="text-[18px] font-bold text-slate-800 dark:text-slate-100 tracking-tight">Welcome to NeuralForge</h1>
                <p className="text-[11.5px] text-slate-500 dark:text-slate-400 mt-0.5">Interactive Neural Network Builder</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2.5 mb-5">
              {[
                { icon: <Layers size={15} className="text-blue-500" />, title: 'Demo Network', desc: 'MNIST classifier pre-loaded — 7 layers, ready to train' },
                { icon: <Zap size={15} className="text-indigo-500" />, title: 'Wire Nodes', desc: 'Right port → left port to connect layers' },
                { icon: <Sparkles size={15} className="text-teal-500" />, title: 'Train & Test', desc: 'Simulate gradient descent with live metrics' },
                { icon: <Download size={15} className="text-blue-500" />, title: 'Export JSON', desc: 'Download TF.js-compatible model definition' },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700/60">
                  <div className="w-7 h-7 rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center flex-shrink-0 shadow-[var(--shadow-xs)]" aria-hidden="true">{item.icon}</div>
                  <div>
                    <div className="text-[12px] font-semibold text-slate-700 dark:text-slate-200">{item.title}</div>
                    <div className="text-[10.5px] text-slate-500 dark:text-slate-400 mt-0.5 leading-tight">{item.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            <div className="flex items-start gap-2.5 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-xl border border-blue-100 dark:border-blue-900/50 mb-4">
              <HelpCircle size={14} className="text-blue-500 dark:text-blue-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
              <p className="text-[11px] text-blue-700 dark:text-blue-300 leading-relaxed">
                An <strong>MNIST simple classifier</strong> (Input → Dense 128 → ReLU → Dropout → Dense 64 → Sigmoid → Output) is pre-loaded.
                Press <kbd className="bg-white dark:bg-blue-900/40 border border-blue-200 dark:border-blue-700 rounded px-1 py-0.5 font-mono text-[10px]">Space</kbd> to start
                training, or drag layers from the sidebar to modify it.
              </p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => { setShowWelcome(false); setShowTutorial(true); }}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-indigo-200 dark:border-indigo-700 text-indigo-600 dark:text-indigo-400 rounded-xl font-semibold text-[12px] hover:bg-indigo-50 dark:hover:bg-indigo-950/30 transition-all"
              >
                <GraduationCap size={14} /> Take Tutorial
              </button>
              <button
                onClick={() => setShowWelcome(false)}
                autoFocus
                className="flex-1 py-2.5 bg-gradient-to-r from-blue-800 to-blue-600 text-white rounded-xl font-semibold text-[13px] hover:from-blue-700 hover:to-blue-500 transition-all shadow-md shadow-blue-800/25 dark:shadow-blue-900/40 active:scale-[0.99] nf-bevel"
              >
                Start Building →
              </button>
            </div>
